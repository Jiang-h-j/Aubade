...
当前整体状态：**全部节点已完成（N00~N07 done）**；N01 = 第一个可自用版本，手动记账 + 账单列表/筛选/编辑/删除闭环；N02 = 剩余金额 + 统计，能看花销与结余；N03 = 第一个 AI 入口，粘贴短信/文本一键识别入账 + 结果卡片改删撤销 + 失败转手动；N04 = 第一个"张口就记"入口，按住说话本机转文字 → 复用 N03 解析链路入账，语音不外传；N05 = 截图相册选图，本机 Vision OCR → 复用 N03 解析入账，图片不外传，产出的 OCR 能力供 N06 复用；N06 = 截图·快捷指令后台入账主入口，App Intents 后台链路 OCR→DeepSeek→入账→本地通知 + 点击深链路由，代码链路与可测部分单测/自评审已通过，**真机专项自测项**：真快捷指令传图、后台时间预算能否容纳一次 DeepSeek 往返、真机通知交付；若真机后台预算不足触发技术基线 §7.3 方案 B 降级；**N07 = 收尾节点，补齐生产配置中心+超支阈值可配、我的页预算/Key 状态/分类只读、首次启动两步引导+根路由分流、通知开关 gating+各权限被拒统一降级，编译通过、单测全绿、两轮自评审 PASS**。**Aubade v1 开发 DAG 全部 8 节点收尾**；剩余为跨节点真机端到端验收（尤其 N06 后台链路、N07 通知开关/权限降级的 UI 观察项）。
...
| N02 | 剩余金额 + 统计 | done | N01 | M5 + M6 | ✅ 能看花销与结余 |
| N03 | DeepSeek 解析 + 文本识别 | done | N00, N01 | M4 + M2.2 | ✅ 粘贴短信一键记账 |
| N04 | 语音记账 | done | N03 | M2.3 | — |
| N05 | 截图·相册选图 | done | N03 | M2.4 | — |
| N06 | 截图·快捷指令后台入账 + 通知 | done | N03, N05 | M2.5 + M7 | ✅ 截图主入口（真机） |
| N07 | 设置/预算/Key/首次引导/权限收尾 | done | N01 | M8 + M9 | ✅ 配置与异常闭环 |
...
- **状态**：done（代码链路 + 可测部分单测/自评审通过；真机专项验证项见"关键约束"，留用户自测，不阻塞节点门禁）
- **依赖**：N03, N05
...
- **状态**：done（4 个 TRD 切片全部完成：配置中心+超支阈值可配、我的页预算/Key/分类只读、首次启动两步引导+根路由分流、通知开关 gating+权限被拒统一降级；编译通过、gating/引导/统计单测全绿、两轮自评审 PASS。UI 端到端建议真机跑一遍确认，不阻塞节点门禁）
