...
当前整体状态：**开发中**；N00、N01、N02、N03、N04、N05 已完成（N01 = 第一个可自用版本，手动记账 + 账单列表/筛选/编辑/删除闭环；N02 = 剩余金额 + 统计，能看花销与结余；N03 = 第一个 AI 入口，粘贴短信/文本一键识别入账 + 结果卡片改删撤销 + 失败转手动；N04 = 第一个"张口就记"入口，按住说话本机转文字 → 复用 N03 解析链路入账，语音不外传；N05 = 截图相册选图，本机 Vision OCR → 复用 N03 解析入账，图片不外传，产出的 OCR 能力供 N06 复用）。下一个可开发节点 **N06 截图·快捷指令后台入账 + 通知**（依赖 N03 + N05 均已满足；★最复杂、必须真机验证，风险隔离在最后）。
